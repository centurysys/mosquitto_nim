...
新規コードでは、これらの helper を `mqttPublishProperties(...)` で `MqttPublishProperties` に変換してから `publishV5()` に渡します。互換性のため従来の `MqttProperties` overload も残しています。
...
このステップで実際に publish API に接続しているのは `MqttPublishProperties` だけです。CONNECT / SUBSCRIBE property の送信は別ステップで追加します。
...
MQTT v5 properties 用に `publishV5()` を追加しています。
