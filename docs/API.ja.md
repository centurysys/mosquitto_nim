...
`MqttConnectProperties` は MQTT v5 CONNECT に接続済みです。`MqttPublishProperties` は publish API に接続済みです。`MqttSubscribeProperties` は次の subscribe-property 対応用の土台です。
...
MQTT v5 CONNECT metadata 用に `setConnectProperties()`、PUBLISH metadata 用に `publishV5()` を追加しています。
